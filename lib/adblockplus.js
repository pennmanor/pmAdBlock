...
require.scopes["stats"] = (function()
{
  ...
  var statsPerTab = new TabMap(true);
  var getStats = exports.getStats = function getStats(key, tab)
  {
    if (!tab)
    {
      return key in Prefs.stats_total ? Prefs.stats_total[key] : 0;
    }
    var tabStats = statsPerTab.get(tab);
  ...
  function forEachTab(func)
  {
    ...
    {
      for (var _loopIndex41 = 0; _loopIndex41 < windows.length; ++_loopIndex41)
      {
        var window = windows[_loopIndex41];
...
require.scopes["whitelisting"] = (function()
{
  ...
  var tabsWithKeyException = new TabMap(true);
  var isWhitelisted = exports.isWhitelisted = function(url, parentUrl, type)
  {
    var filter = defaultMatcher.matchesAny(stripFragmentFromURL(url), type || "DOCUMENT", extractHostFromURL(parentUrl || url), false);
    return filter instanceof WhitelistFilter ? filter : null;
  };
  var isFrameWhitelisted = exports.isFrameWhitelisted = function(tab, frame, type)
  {
    var urlsWithKeyException = tabsWithKeyException.get(tab);
  ...
  var verifyKeyException = function(token, url, docDomain)
  {
    var match = token.match(/((.*?)=*)_(.*)/);
    if (!match)
    {
      return false;
    }
    var strippedKey = match[2];
    ...
    var key = match[1];
    var signature = match[3];
    return verifySignature(key, signature, params.join("\x00"));
  };
  var recordKeyException = function(tab, url)
  {
    var urlsWithKeyException = tabsWithKeyException.get(tab);
    if (!urlsWithKeyException)
    {
      urlsWithKeyException =
      {
        __proto__: null
      };
  ...
  var processKeyException = exports.processKeyException = function(token, tab, frame)
  {
